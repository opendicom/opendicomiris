      ...
      if( [[selectedLine valueForKey:@"type"] isEqualToString: @"Series"])
		...
		else
		{
         ...
         if(([@[@"OT",@"DOC"]indexOfObject:[toOpenArray[0][0] valueForKey:@"storedModality"]]!=NSNotFound) && [[toOpenArray[0][0] sopInstanceUID] hasPrefix:@"2"])
         {
            ...
            NSRange xmlBeginRange=[enclosingData rangeOfData:xmlData options:0 range:enclosingRange];
            
            NSData *xmlEnd=[@">" dataUsingEncoding:NSASCIIStringEncoding];
            NSRange xmlEndRange=[enclosingData rangeOfData:xmlEnd options:NSDataSearchBackwards range:enclosingRange];
            
            [[enclosingData subdataWithRange:NSMakeRange(xmlBeginRange.location, xmlEndRange.location-xmlBeginRange.location+1)] writeToFile:@"/tmp/cda.xml" atomically:NO];
            
            //[[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:@"/Users/Shared/.cda.xml"]];
            [[NSWorkspace sharedWorkspace] openFile:@"/tmp/cda.xml" withApplication:@"safari"];

         }
			else [self processOpenViewerDICOMFromArray: toOpenArray movie: movieViewer viewer: viewer];
		}
