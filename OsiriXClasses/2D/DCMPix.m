...
+ (void) checkUserDefaults: (BOOL) update
{
	...
	if( gUserDefaultsSet == NO)
	{
		...
		gUSEPAPYRUSDCMPIX = YES; //[[NSUserDefaults standardUserDefaults] boolForKey:@"USEPAPYRUSDCMPIX4"];
...
//                                                                    NSLog(@"...... csu...");
		...
		#pragma mark read pixel data
				...
				if( fileNb >= 0)
				{
					...
					{
                  NSLog(@"DCMPix EncapsulatedCDAStorage");
                  if([modalityString isEqualToString:@"DOC"])
                  [self getDataFromNSImage: [NSImage imageNamed: @"doc.png"]];
                  else [self getDataFromNSImage: [NSImage imageNamed: @"cal.png"]];
                  returnValue = YES;
               }
...
#pragma mark - dicom
		if( [self isDICOMFile: sourceFile])
		{
			...
			@try
			{
               ...
               if( numberOfFrames <= 1) [self clearCachedPapyGroups];
			}
		...
		if( success == NO)	// Is it a NON-DICOM IMAGE ??
		{
         ...
         id fileFormatClass = PluginManager.fileFormatClasses[[sourceFile pathExtension]];
			if (fileFormatClass)
			{
#pragma mark fileFormat (indexed by extension) from plugin
...
#pragma mark .hdr
         ...
         {
#pragma mark .jpg .jp2 .jpeg .pdf .pct .png .gif
